...
public class RestoreHbpressure extends Activity {
	...
	private static final String SYS = "Systolic";
	private static final String DIAS = "Diastolic";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		db = new DatabaseManipulator(RestoreHbpressure.this);
		db.deleteData4();
	...
	class RestoreAll extends AsyncTask<String, String, String> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = new ProgressDialog(RestoreHbpressure.this);
			pDialog.setMessage("Restoring HbP BackUp. Please wait...");
		...
		protected String doInBackground(String... args) {
			...
			url = "http://10.0.2.2/diamed/mobile/getHbpBackup.php";
			...
			try {
				...
				if (success == 1) {
					...
					for (int i = 0; i < result.length(); i++) {
						...
						String systolic = c.getString(SYS);
						String diastolic = c.getString(DIAS);

						db.insert4(date, Time, activity, systolic, diastolic);
		...
		protected void onPostExecute(String file_url) {
			...
			runOnUiThread(new Runnable() {
				public void run() {
					...
					RestoreHbpressure.this.startActivity(i);
